...
function assertEip712Transaction(transaction) {
  const { chainId, to, from: from11, paymaster, paymasterInput } = transaction;
...
function serializeTransactionEIP712(transaction) {
  const { chainId, gas, nonce, to, from: from11, value, maxFeePerGas, maxPriorityFeePerGas, customSignature, factoryDeps, paymaster, paymasterInput, gasPerPubdata, data } = transaction;
...
function transactionToMessage(transaction) {
  const { gas, nonce, to, from: from11, value, maxFeePerGas, maxPriorityFeePerGas, factoryDeps, paymaster, paymasterInput, gasPerPubdata, data } = transaction;
  return {
    txType: 113n,
    from: BigInt(from11),
    to: to ? BigInt(to) : 0n,
...
function serializeTransactionDeposit(transaction) {
  ...
  const { sourceHash, data, from: from11, gas, isSystemTx, mint: mint2, to, value } = transaction;
  const serializedTransaction = [
    sourceHash,
    from11,
    to ?? "0x",
...
function assertTransactionDeposit(transaction) {
  const { from: from11, to } = transaction;
  if (from11 && !isAddress(from11))
    throw new InvalidAddressError({ address: from11 });
  if (to && !isAddress(to))
    throw new InvalidAddressError({ address: to });
...
function verify2(options) {
  ...
  return secp256r12.verify(signature, payload instanceof Uint8Array ? payload : fromHex(payload), toHex2(publicKey).substring(2), ...hash4 ? [{ prehash: true, lowS: true }] : []);
}
...
function verify3(options) {
  ...
  if (fromBytes(toBytes2(challenge_extracted)) !== challenge)
    return false;
...
function compute(value) {
  const hash4 = keccak256(encode(from2("address, bytes32"), [
    value.sender,
    value.salt
  ]));
...
function from10(envelope, options = {}) {
  ...
  return {
    ...
    ...feePayerSignature ? { feePayerSignature: from3(feePayerSignature) } : {},
    type: "tempo"
  };
}
...
async function serializeTempo(transaction, sig) {
  ...
  if (signature && typeof transaction.feePayer === "object") {
    ...
    return TxEnvelopeTempo_exports.serialize(tx, {
      feePayerSignature: from3(feePayerSignature2)
    });
  }
  if (feePayer === true) {
...
// node_modules/viem/_esm/tempo/internal/nonceKeyStore.js
var store = create();
function create() {
...
function getEntry(store2, parameters) {
  const key = getCacheKey(parameters);
  let entry = store2.get(key);
  if (!entry) {
...
var chainConfig5 = {
  ...
  prepareTransactionRequest: [
    async (r, { phase }) => {
      ...
      if (phase === "afterFillParameters") {
        if (request.feePayer && ((_a = request.keyAuthorization) == null ? void 0 : _a.signature.type) === "webAuthn")
      ...
      request.nonceKey = (() => {
        ...
        const address = ((_a2 = request.account) == null ? void 0 : _a2.address) ?? request.from;
        if (!address)
          return void 0;
        if (!request.chain)
          return void 0;
      ...
      if (!request.feeToken && ((_b = request.chain) == null ? void 0 : _b.feeToken))
  ...
  serializers: {
    ...
    transaction: (transaction, signature) => serialize4(transaction, signature)
  },
  async verifyHash(client, parameters) {
    const { address, hash: hash4, signature } = parameters;
